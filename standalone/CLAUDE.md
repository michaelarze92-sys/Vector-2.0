...
## Branding — shares the Ledger's mark, this app's own colour

The sidebar (`.nav-brand`) and the four `icon-*.png` files use the same brush-stroke
"M" mark as the Estates Ledger (`src/ledger/assets/logos/mark_cutout.png` — a
transparent cutout, gold-on-transparent, no other colour in it), not a separate mark
drawn for this app. Only the **background** differs: `#123526`, this app's own felt
green, in place of the Ledger's crimson (`#2B0A10`) — same composition as
`src/ledger/make-icons.py` (identical size/fraction targets, identical maskable
55%-safe-zone logic for the launcher-crop icon), just a different `BG` constant. There
is no `make-icons.py` equivalent checked in for this app — the icons here were
generated by a one-off script during that session, not by a repeatable build step
(this file has no build step at all, by design). If the mark or the green needs to
change again, regenerate from `mark_cutout.png` with fresh `BG`/fraction values rather
than hand-editing the PNGs.

The sidebar mark is the same source PNG downsized to 96px and inlined as a `data:`
URI directly in `estate-pm.html` (`<img class="brand-mark" ...>` in `.nav-brand`) —
not a separate file, since this file has no build step to inline one at deploy time
the way the Ledger's `__MARK_LOGO__` placeholder does. `alt=""` is deliberate: it's
